...
def search_contact_by_name(first_name):
    ...
    for c in contacts:
                # Match on firstName and the [HGov] lastName tag we set
                if c.get("firstName", "")[:40] == first_name[:40] and c.get("lastName") == "[HGov]":
                                return c.get("id")
    return None

def get_or_create_contact(name):
        """Return existing contact ID if one already exists for this opportunity name,
            otherwise create a new placeholder contact and return its ID."""
    ...
    r = requests.post("https://services.leadconnectorhq.com/contacts/",
                                            ...
                                            json={"firstName": name[:40], "lastName": "[HGov]",
                                                                              "locationId": LOCATION_ID, "tags": ["highergov"]})
...
def sync():
    ...
    for hgov_name, pipeline_pursuits in by_pipeline.items():
        ...
        for p in pipeline_pursuits:
            ...
            if existing:
                                # Reuse the contact already linked to this opportunity — no new contact needed
                                existing_contact_id = existing.get("contactId") or existing.get("contact", {}).get("id")
                ...
                # New opportunity — get or create a contact (avoids duplicates on re-runs)
