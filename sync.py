...
# GHL pipeline config — only "Added to Pipeline" stage ID needed per pipeline
# All new opportunities land in "Added to Pipeline" regardless of HigherGov stage.
# Existing opportunities are NEVER touched — users own stage movement inside GHL.
PIPELINE_CONFIG = {
            "ATW Procurement": {
...
def get_existing_opps(pipeline_id):
            """Return sets of solicitation IDs and opportunity names already in this GHL pipeline.
                We only need these to decide whether to SKIP (never update) an existing opp.
                    """
...
def sync():
    ...
    for hgov_pipeline_name, pipeline_pursuits in by_pipeline.items():
        ...
        # Fetch existing opps — we only use these to detect duplicates and skip them
        ...
        for p in pipeline_pursuits:
            ...
            # --- SKIP if already in GHL (by sol ID or by name) ---
            # We NEVER update existing opps — users own stage/status in GHL.
            ...
            # --- NEW opportunity — create it in "Added to Pipeline" ---
    ...
    print(f"\nSync complete: {created} created, {skipped} skipped (already in GHL), {errors} errors")
